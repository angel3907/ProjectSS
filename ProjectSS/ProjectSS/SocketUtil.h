...
public:
	...
	static uint32_t SendPacket(UDPSocketPtr Socket, SocketAddress& ToAddress, const vector<GameObject*>& InGameObjects, ReplicationManager& InReplicationManager);
	static uint32_t ReceivePacket(UDPSocketPtr Socket, ReplicationManager& InReplicationManager);

	//RA_PRC면 InGameObject가 null, RPCParam 존재 / RA_RMI면 InGameObject가 존재, RPCParam 존재 / 그 외에는 RPCParams만 null
	static uint32_t SendReplicated(UDPSocketPtr Socket, SocketAddress& ToAddress, ReplicationAction InReplicationAction, GameObject* InGameObject, RPCParams* InRPCParams, ReplicationManager& InReplicationManager);
	static uint32_t ReceiveReplicated(UDPSocketPtr Socket, ReplicationManager& InReplicationManager);
